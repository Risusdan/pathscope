"""Firmware trace-buffer test double (design doc section 3). Stands in
for a real target's trace peripheral in tests: it interprets writes
into the descriptor's watch-table fields the way real firmware would
(count=0 closes the gate, a 0->N count transition validates the
pending addresses against a whitelist and, on success, bumps
generation and starts sampling), and step() advances the ring the way
the periodic ISR would.
